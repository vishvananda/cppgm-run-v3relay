...
ExpressionAnalyzer::ExpressionAnalyzer(const vector<Pa6Token>& tokens,
    ...
    : tokens_(tokens), arena_(arena), model_(model), types_(model.Types()),
      tree_(tree), builder_(builder), builtin_functions_(BUILTIN_KIND_COUNT, 0)
{
}
                                            ...
                                            CallResolution& result)
{
  ...
  if (result.member_callee) {
    ...
    if (tree_.At(member).kind == SEMA_MEMBER && tree_.At(member).binding != 0 &&
        model_.BindingAt(tree_.At(member).binding).kind != BINDING_FUNCTION)
    {
      // A data member (a function pointer field) is not a member function
      // candidate: the call goes through the member's value.
      result.indirect_callee = member;
      result.member_callee = false;
      result.named_callee = false;
      return;
    }
    ...
    CallResolution& resolution, const vector<SemaId>& analyzed_arguments)
{
  ...
  const BuiltinFunctionKind builtin_kind = builtin_name ?
      BuiltinKindOf(name.Last()) : BUILTIN_NONE;
  ...
  else if (builtin_kind != BUILTIN_NONE)
  {
    function = BuiltinFunction(builtin_kind);
    function_type = model_.FunctionAt(function).type;
  }
  ...
  else
  {
    ...
    indirect_callee = resolution.indirect_callee != 0 ?
        resolution.indirect_callee : Analyze(callee, scope);
